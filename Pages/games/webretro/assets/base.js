...
var pdKeys = [
	8, 9, 13, 19, 27, 32, 33, 34, 35, 36, 42, 44, 45, 91, 92, 93, 112, 113, 114,
...
var sramExt = '.srm';
var smasBrickFix = {
	'16a160ddd431a3db6fcd7453ffae9c4c': [
		80, 65, 84, 67, 72, 0, 127, 160, 0, 8, 169, 1, 133, 160, 141, 0, 22, 107, 1,
		191, 182, 0, 4, 34, 160, 255, 0, 6, 189, 164, 0, 4, 34, 160, 255, 0, 69, 79,
		70,
	],
	e87d43969bdf563d1148e3b35e8b5360: [
		80, 65, 84, 67, 72, 0, 129, 160, 0, 8, 169, 1, 133, 160, 141, 0, 22, 107, 1,
		193, 182, 0, 4, 34, 160, 255, 0, 6, 191, 164, 0, 4, 34, 160, 255, 0, 69, 79,
		70,
	],
	'2071b049a463cefd7a0b7aeab8037ca0': [
		80, 65, 84, 67, 72, 0, 127, 160, 0, 8, 169, 1, 133, 160, 141, 0, 22, 107, 1,
		191, 190, 0, 4, 34, 160, 255, 0, 6, 189, 164, 0, 4, 34, 160, 255, 0, 69, 79,
		70,
	],
...
// make core lists
var aCoreList =
	'<li><b>Select a Core</b></li><li><a href="?core=autodetect" class="greyer">AutoDetect (Slower to load)</a></li>';
for (var i = 0; i < installedCores.length; i++) {
	aCoreList +=
		'<li><a href="?core=' +
		installedCores[i] +
...
function fakeCharPress(key) {
	if (charToCodeMap.hasOwnProperty(key))
		fakeKeyPress({
...
async function tryLsToIdb() {
	...
	for (var i = 0; i < ls.length; i++) {
		if (ls[i].startsWith('RetroArch_saves_')) {
			setIdbItem(
				ls[i],
				new Uint8Array(JSON.parse(window.localStorage.getItem(ls[i])))
			);
...
function setStatus(message) {
	...
	canvas.style.backgroundImage =
		...
		btoa(
			'<svg xmlns="http://www.w3.org/2000/svg" width="150" height="150"><text style="font: 30px sans-serif;" fill="white" x="50%" y="40%" dominant-baseline="middle" text-anchor="middle">Loading</text><text style="font: 15px sans-serif;" fill="white" x="50%" y="60%" dominant-baseline="middle" text-anchor="middle">' +
				message +
				'</text></svg>'
		) +
		'")';
...
function checkForUpdates() {
	grab(
		'https://cdn.jsdelivr.net/gh/BinBashBanana/webretro@latest/assets/info.json',
		'text',
		function (text) {
			try {
				...
				if (updateObj.webretro) {
					...
					if (updateObj.versions[webretroVersion.toString()])
						versionIndicator.title =
							'New features in this version:\n\n- ' +
							updateObj.versions[webretroVersion.toString()].changeList.join(
								'\n- '
							);
					if (
						latestVersion > webretroVersion &&
						updateObj.versions[latestVersion.toString()]
					) {
						updateNotice.textContent =
							'New webretro version available: v' +
...
function unzipFile(data, exts, callback, empty, notfound) {
	new zip.ZipReader(new zip.Uint8ArrayReader(data))
		.getEntries()
				...
				for (var i = 0; i < entries.length; i++) {
					if (
						exts
							.split(', ')
							.includes(
								'.' + u8atoutf8(entries[i].rawFilename).split('.').slice(-1)[0]
							)
					) {
...
async function zipFiles(files, callback, replaceName) {
	...
	for (var i = 0; i < files.length; i++) {
		await writer.add(
			replaceName ? files[i].name.replace('rom', romName) : files[i].name,
			new zip.Uint8ArrayReader(files[i].data)
		);
	}
	await writer.close();
...
function handleWebFile(data) {
	if (data.message == 'success') {
		...
		alert(
			'There was an error with the file picker. This may mean that you have to allow popup windows.'
		);
	}
}
...
function readyRomFetch() {
	var romloc = /^(http:\/\/|https:\/\/|\/\/)/i.test(queries.rom)
		? queries.rom
		: 'roms/' + queries.rom;
...
wconsole.wconsoleinput.title =
	'You can type things here as though you were using the browser console.';
...
if (queries.hasOwnProperty('console'))
	conw.open({ width: 450, height: 250, left: 100, top: 50 });
...
var savedKeybindsObj = window.localStorage.getItem(
	'RetroArch_settings_keybinds'
)
	? configStrToObj(window.localStorage.getItem('RetroArch_settings_keybinds'))
	: Object.assign({}, defaultKeybindsObj);
...
function createConfigList() {
	...
	for (var i = 0; i < validKeybinds.length; i++) {
		...
		if (matches.length > 1 && !(matches[0] == 'nul'))
			keybindTable.children[i].lastElementChild.classList.add('conflict');
	}
}
...
keybindTable.onclick = function (e) {
	if (e.target.tagName == 'TD' && !e.target.nextElementSibling) {
		...
		let keyNo = Array.from(keybindTable.children).indexOf(
...
function tryApplyConfig() {
	if (emulatorStarted) {
		FS.writeFile(
			...
			nulKeys + configObjToStr(savedKeybindsObj) + extraConfig
		);
...
saveKeybinds.onclick = function () {
	...
	window.localStorage.setItem(
		'RetroArch_settings_keybinds',
		configObjToStr(savedKeybindsObj)
	);
	tryApplyConfig();
	alert('Saved!');
};
...
resetKeybinds.onclick = function () {
	if (
		confirm(
			'Are you sure you want to reset all of the keybinds to their default values?'
		)
	) {
...
function createScreenshotList() {
	var screenshots = FS.analyzePath(
		'/home/web_user/retroarch/userdata/screenshots/'
	).exists
		? FS.readdir('/home/web_user/retroarch/userdata/screenshots/').filter(
				k => !['.', '..'].includes(k)
		  )
		: [];
	...
	for (var i = 0; i < screenshots.length; i++) {
		var screenshotData = FS.readFile(
			'/home/web_user/retroarch/userdata/screenshots/' + screenshots[i]
		);
		var blobUrl = window.URL.createObjectURL(
			new Blob([screenshotData], { type: 'image/png' })
		);
		...
		screenshotsDiv.innerHTML +=
			...
			blobUrl +
			'"><input type="button" data-action="download" value="Download"><input type="button" data-action="delete" value="Delete">' +
			'</div>';
	}
}
...
screenshotsDiv.onclick = function (e) {
	if (e.target.tagName == 'INPUT') {
		var screenshotNo = Array.from(screenshotsDiv.children).indexOf(
			e.target.parentElement
		);
		switch (e.target.dataset.action) {
			case 'download':
				downloadFile(
					screenshotDatas[screenshotNo].data,
					screenshotDatas[screenshotNo].name.replace('rom', romName),
					'image/png'
				);
				break;
			case 'delete':
				if (confirm('Are you sure you want to delete this screenshot?')) {
					...
					FS.unlink(
						'/home/web_user/retroarch/userdata/screenshots/' +
							screenshotDatas[screenshotNo].name
					);
...
function updateQuotaDisplay() {
	navigator.storage.estimate().then(function (info) {
		quotaText.textContent =
			...
			bytesToHumanReadable(info.usage) +
			' / ' +
			bytesToHumanReadable(info.quota) +
			' (' +
			(info.usage / info.quota).toFixed(2) +
			'%)';
	});
}
		...
		for (var i = 0; i < items.length; i++) {
			if (/^RetroArch_(saves|states)_/.test(items[i].key)) {
				...
				saveTable.innerHTML +=
					...
					': ' +
					sName +
					'</td><td><span data-action="download">Download</span><span data-action="delete">Delete</span></td></tr>';
			}
		}
	});
}
...
saveTable.onclick = function (e) {
	if (e.target.tagName == 'SPAN') {
		let saveNo = Array.from(saveTable.children).indexOf(
				...
				getIdbItem(saveIDs[saveNo].id).then(function (data) {
					downloadFile(
						...
						'game-' +
							...
							'-' +
							getTime() +
							(saveIDs[saveNo].type == 'save' ? '.srm' : '.state')
					);
				});
				break;
			case 'delete':
...
(function () {
	...
	if (!queries.core && queries.system) {
		var detectedCore = Object.keys(systems).find(
			k => systems[k].toLowerCase() == queries.system.toLowerCase()
		);
		...
		if (queries.core.toLowerCase() == 'autodetect') {
			...
			getCore(
				...
				function () {
					// core loading error
					alert(
						'Could not load specified core "' +
							core +
							'". Here is a list of available cores.'
					);
...
function initFromFile(name, data) {
	...
	if (name.split('.').slice(-1)[0] == 'zip') {
		...
		unzipFile(
			...
			function () {
				alert(
					"Couldn't find a valid ROM file in that zip file. Are you using the right core? This is " +
						systems[core] +
						'. (The ROM has to be at the base directory of the zip file)'
				);
			}
		);
	} else {
...
function autodetectCoreHandler(name, data) {
	...
	if (name.split('.').slice(-1)[0] == 'zip') {
		...
		unzipFile(
			...
			function () {
				alert(
					"Couldn't find a valid ROM file in that zip file. (The ROM has to be at the base directory of the zip file)"
				);
			}
		);
	} else {
...
function autodetectCore(name, data) {
	...
	for (var i = 0; i < fileExtsArray.length; i++) {
		if (fileExts[fileExtsArray[i]].split(', ').includes(nameExt)) {
			detectedCore = Object.keys(systems).find(
				k => systems[k] == fileExtsArray[i]
			);
			break;
		}
	}
...
function prepareBundle() {
	...
	grab(
		bundleCdnLatest + 'bundle/indexedfiles-v2.txt',
		'text',
		function (data) {
			...
			for (var i = 0; i < fsBundleDirs.length; i++) {
				FS.createPath(
					baseFsBundleDir + fsBundleDirs[i][0],
					fsBundleDirs[i][1],
					true,
					true
				);
			...
			for (let i = 0; i < fsBundleFiles.length; i++) {
				grab(
					bundleCdn + 'bundle' + fsBundleFiles[i],
					'arraybuffer',
					function (data) {
						FS.writeFile(
							baseFsBundleDir + fsBundleFiles[i],
							new Uint8Array(data)
						);
						if (i == fsBundleFiles.length - 1)
							donePreparingBundle(performance.now() - bundleSTime);
					},
					function () {
						bundleErrors += 1;
						if (i == fsBundleFiles.length - 1)
...
function donePreparingBundle(tooktime) {
	...
	removeStatus('Getting assets');
	log(
		'Finished bundle fetch in ' +
			(tooktime / 1000).toFixed(1) +
			' seconds, ' +
			bundleErrors +
			' errors'
	);
...
function doNotRename() {
	if (
		romMode == 'upload' &&
		!window.localStorage.getItem('webretro_settings_pastFirstSave')
	) {
		alert(
			'WARNING: Do not rename your ROM file after this! The save data is specific to the ROM name!'
		);
...
function saveSRAM() {
	...
	window.setTimeout(function () {
		if (
			FS.analyzePath('/home/web_user/retroarch/userdata/saves/rom' + sramExt)
				.exists
		) {
			setIdbItem(
				'RetroArch_saves_' + romName,
				FS.readFile('/home/web_user/retroarch/userdata/saves/rom' + sramExt)
			);
...
function saveStateFunc() {
	window.setTimeout(function () {
		if (
			FS.analyzePath('/home/web_user/retroarch/userdata/states/rom.state')
				.exists
		) {
			setIdbItem(
				'RetroArch_states_' + romName,
		...
		document.addEventListener(
			...
			function (e) {
				if (
					!stateReaders2Ready &&
					e.code ==
						Object.keys(codeToConfigIDMap).find(
							k => codeToConfigIDMap[k] == savedKeybindsObj.input_load_state
						)
				)
					readyStateReaders2();
			},
...
function readySaveReaders() {
	if (!saveReadersReady) {
		...
		exportSave.onclick = function () {
			downloadFile(
				FS.readFile('/home/web_user/retroarch/userdata/saves/rom' + sramExt),
				'game-sram-' + romName + '-' + getTime() + sramExt
			);
		};
	}
}
...
function afterStart() {
	...
	importState.onclick = function () {
		uploadFile(
			'.bin, .state, .save, .dat, .gam, .sav, application/*',
			function (file) {
	...
	importSave.onclick = function () {
		uploadFile(
			'.bin, .srm, .sram, .ram, .gam, .sav, .dsv, application/*',
			function (file) {
				autosave.checked = false;
				setIdbItem('RetroArch_saves_' + romName, new Uint8Array(file.data));
				if (
					confirm('Save imported. Reloading now for changes to take effect.')
				) {
	...
	document.addEventListener(
		...
		function (e) {
			if (
				...
				e.code ==
					Object.keys(codeToConfigIDMap).find(
						k => codeToConfigIDMap[k] == savedKeybindsObj.input_save_state
					)
			)
				readyStateReaders();
		},
	...
	async function waitForReady() {
		if (wasmReady && bundleReady) {
			...
			if (cSave) {
				...
				FS.writeFile(
					'/home/web_user/retroarch/userdata/saves/rom' + sramExt,
					cSave
				);
